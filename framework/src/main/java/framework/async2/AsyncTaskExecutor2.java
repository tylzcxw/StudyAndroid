...
public class AsyncTaskExecutor2 {
    /** 任务执行延迟毫秒数，为了在此期间可以取消该任务考虑 */
    private static final long DELAY_MILLISECONDS = 0;
    ...
    private static AsyncTaskExecutor2 mInstance;
    private ScheduledThreadPoolExecutor mExecutor = null;
    private ScheduledThreadPoolExecutor mLowPriorityExecutor = null;
    public static synchronized AsyncTaskExecutor2 getInstance(){
        if(mInstance == null){
            mInstance = new AsyncTaskExecutor2();
        }
        return mInstance;
    }
    private AsyncTaskExecutor2(){
     ...
     * @param task
     */
    public void execute(AsyncTask2 task){
     ...
     * @param task
     */
    public void executeLowPriorityTask(AsyncTask2 task) {
