...
public class StickyListHeadersListView extends ListView {
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (!(adapter instanceof StickyListHeadersAdapter)) {
			throw new IllegalArgumentException(
					"Adapter must implement CcbStickyListHeadersAdapter");
		}
	...
	private void measureHeader() {
		...
		int widthMeasureSpec = View.MeasureSpec.makeMeasureSpec(getWidth()
		...
		if (params != null && params.height > 0) {
			heightMeasureSpec = View.MeasureSpec.makeMeasureSpec(params.height,
																 View.MeasureSpec.EXACTLY);
		} else {
